...
CLASSIFY_REFRESH_SEC = 3.0        # unconfirmed answers retry sooner (was 5)
STICKY_CONFIRMED_SEC = 300.0      # confirmed labels stick for 5 min — tools on
                                  # a table don't change identity; stops the
                                  # "keeps re-asking forever" slowdown
CLASSIFY_MAX_PER_PASS = 3          # more tracks resolved per pass (was 2)
IOU_MATCH = 0.3

# CPU budget — FULL-THROTTLE mode: with the prototype fast path active, the
# ViT classifier only runs for unsure tracks, so both models can now use the
# whole Pi. ORT sessions with intra_op=4 split physical cores only when both
# run simultaneously; OMP threads env + gunicorn threads handle the rest.
DETECT_THREADS = 4
CLASSIFY_THREADS = 4
        ...
        for inst in insts:
            ...
            if cached is not None:
                ...
                if (now - cached["ts"]) < CLASSIFY_REFRESH_SEC:
                    continue
            # every label comes from the ViT (fast path removed: prototypes
            # confused 23/150 as Triple_Syringe in field tests)
